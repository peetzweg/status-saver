...
func TestSenderLabelFor(t *testing.T) {
	jid := types.NewJID("4915164300143", types.DefaultUserServer)
	if got := senderLabelFor(jid, "Philip"); got != "Philip_4915164300143" {
		t.Errorf("with push name: %q", got)
	}
	if got := senderLabelFor(jid, ""); got != "4915164300143" {
