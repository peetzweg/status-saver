// Package daemon implements the `status-saver run` subcommand — the
// long-running archiver daemon.
package daemon
...
// Run is the entry point for `status-saver run`. Returns an exit code.
// A permanent WhatsApp logout returns 1 (systemd uses RestartPreventExitStatus=1
// to avoid restart loops); configuration / startup failures return 2.
func Run(args []string) int {
	fs := flag.NewFlagSet("run", flag.ExitOnError)
	fs.Usage = func() {
		fmt.Fprintln(fs.Output(), "Usage: status-saver run [flags]")
		fmt.Fprintln(fs.Output())
		fmt.Fprintln(fs.Output(), "Long-running archiver daemon. Listens for status@broadcast")
		fmt.Fprintln(fs.Output(), "messages and writes them to disk.")
		fmt.Fprintln(fs.Output())
		fs.PrintDefaults()
	}
